...
import me.coolmint.ngm.event.impl.EventPostSync;
import me.coolmint.ngm.event.impl.EventSync;
import me.coolmint.ngm.features.settings.Setting;
import me.coolmint.ngm.util.models.Timer;
import me.coolmint.ngm.util.player.InteractionUtility;
import me.coolmint.ngm.util.player.PlayerUtility;

import static me.coolmint.ngm.util.player.InteractionUtility.BlockPosWithFacing;
import static me.coolmint.ngm.util.player.InteractionUtility.checkNearBlocks;

public class Scaffold extends Module {
    private final Setting<InteractionUtility.PlaceMode> placeMode = register(new Setting<>("PlaceMode", InteractionUtility.PlaceMode.Normal));
    public Setting<Boolean> rotate = register(new Setting<>("Rotate", true));
    public Setting<Boolean> allowShift = register(new Setting<>("AllowShift", false));
    public Setting<Boolean> autoswap = register(new Setting<>("AutoSwap", true));
    public Setting<Boolean> tower = register(new Setting<>("Tower", true));
    public Setting<Boolean> safewalk = register(new Setting<>("SafeWalk", true));
    public Setting<Boolean> echestholding = register(new Setting<>("EchestHolding", false));
    ...
    public Scaffold() {
        super("Scaffold", "", Module.Category.MOVEMENT, true, false, false);
    }
