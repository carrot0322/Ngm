...
import me.coolmint.ngm.util.player.Position;
...
public class TpAura extends Module {
    ...
    public void move(LivingEntity target, Runnable attack) {
        Position pos = new Position(mc.player.getX(), mc.player.getY(), mc.player.getZ());
        byte var5;
        if ((Boolean)this.motion.getValue()) {
            EntityUtils.motion(target.getX(), target.getY(), target.getZ());
        } else {
        ...
        attack.run();
        if ((Boolean)this.motion.getValue()) {
            EntityUtils.motion(pos.getX(), pos.getY(), pos.getZ());
        } else {
            switch (this.type.getValue()) {
                case Full:
